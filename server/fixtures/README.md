...
| `work-oneshot-3694820.html` | 1/1 finished the day it was posted, so AO3 prints no status row; also restricted, so the title carries a lock `<img>`; single-chapter, so there is no chapter menu |
...
The chapter menu (`<select id="selected_id">`) sits above that cut and must
survive it: the parser reads it to turn a chapter id into a chapter number. A
multi-chapter fixture should hold one `<option>` per posted chapter.
